...
public class PublishHandler extends AbstractMqttMessageHandler<MqttPublishMessage> {
    ...
    private Mono<Void> handle0(MqttMessageContext<MqttPublishMessage> messageContext, MqttSession mqttSession, MqttBrokerContext brokerContext) {
        ...
        switch (qos) {
            case AT_MOST_ONCE:
                return MqttPublishMessageHelper.broadcastAndSaveIfRetain(brokerContext, mqttSession, pubTopic, messageContext);
            case AT_LEAST_ONCE:
                return MqttPublishMessageHelper.broadcast(brokerContext, mqttSession, pubTopic, messageContext)
